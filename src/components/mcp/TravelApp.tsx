        ...
        <div className="bg-[#0f1523] rounded-[1.35rem] p-5 relative overflow-hidden flex-1 flex flex-col">
            ...
            <div>
              <h2
                ...
                style={{
                  ...
                  backgroundImage: 'linear-gradient(to right, #fb7185, #fca5a5)',
                }}
              >
          ...
          {step === 'form' && (
            <form onSubmit={handleSubmit} className="flex-1 flex flex-col gap-4 relative z-10">
              <div className="space-y-3 flex-1 overflow-y-auto pr-1">
                <div>
                  <label className="block text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1.5 ml-1">
                    Destino
                  </label>
                  <div className="relative">
                    ...
                    <input
                      required
                      type="text"
                      className="w-full bg-[#1e293b]/50 border border-white/10 rounded-xl py-2.5 pl-10 pr-4 text-white text-sm focus:outline-none focus:border-rose-400/50 focus:bg-[#1e293b] transition-colors placeholder:text-gray-600"
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <select
                        className="w-full bg-[#1e293b]/50 border border-white/10 rounded-xl py-2.5 pl-10 pr-4 text-white text-sm focus:outline-none focus:border-rose-400/50 focus:bg-[#1e293b] transition-colors appearance-none"
                        value={budget}
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <textarea
                      rows={2}
                      className="w-full bg-[#1e293b]/50 border border-white/10 rounded-xl py-2.5 pl-10 pr-4 text-white text-sm focus:outline-none focus:border-rose-400/50 focus:bg-[#1e293b] transition-colors placeholder:text-gray-600 resize-none"
              ...
              <div className="relative group mt-2 shrink-0">
                ...
                <button
                  ...
                  className="relative w-full bg-[#1e293b]/50 hover:bg-[#1e293b] text-white py-3.5 px-4 rounded-xl flex items-center justify-center gap-2 transition-all duration-200 border border-white/10 shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
                >
          ...
          {step === 'loading' && (
            <div className="flex-1 flex flex-col items-center justify-center gap-4 py-8 relative z-10">
              <div className="relative">
                <span className="material-symbols-rounded animate-spin text-rose-400 text-[48px]">autorenew</span>
                <span className="material-symbols-rounded absolute inset-0 flex items-center justify-center text-orange-300 text-[24px]">airplanemode_active</span>
              </div>
              <div className="text-center space-y-1">
                <h3 className="text-white font-semibold">Diseñando tu viaje ideal</h3>
                <p className="text-gray-500 text-sm">Consultando agentes expertos...</p>
              </div>
            </div>
          )}
          ...
          {step === 'error' && (
            <div className="flex-1 flex flex-col items-center justify-center gap-4 py-8 relative z-10 text-center">
              <div className="w-16 h-16 rounded-full bg-red-500/10 flex items-center justify-center">
                <span className="material-symbols-rounded text-red-400 text-[36px]">error</span>
              </div>
              <div>
                <h3 className="text-white font-semibold text-lg">Vaya, hubo un problema</h3>
                <p className="text-gray-400 text-sm mt-1">{errorMsg}</p>
              </div>
              <button
                onClick={() => setStep('form')}
                className="mt-2 bg-white/10 hover:bg-white/15 text-white py-2 px-6 rounded-lg text-sm font-medium transition-colors"
              >
                Intentar de nuevo
              </button>
            </div>
          ...
          {step === 'results' && (
            <div className="flex-1 flex flex-col relative z-10 h-full overflow-hidden">
                ...
                {suggestions.map((sug, i) => (
                  <div key={sug.id || i} className="bg-[#1e293b]/30 border border-white/10 rounded-2xl p-4 hover:bg-[#1e293b]/50 transition-colors">
                    <div className="flex justify-between items-start gap-2 mb-2">
                      <h4 className="text-white font-bold text-sm leading-tight">{sug.title}</h4>
                      <span className="bg-emerald-500/20 text-emerald-300 text-xs font-bold px-2 py-0.5 rounded-full shrink-0">
                        {sug.estimatedCost}
                      </span>
                    </div>
                    <p className="text-gray-400 text-xs leading-relaxed mb-3">{sug.description}</p>

                    {sug.highlights && Array.isArray(sug.highlights) && (
                      <div className="flex flex-wrap gap-1.5">
                        {sug.highlights.map((hl, j) => (
                          <span key={j} className="bg-white/5 text-gray-300 text-[10px] uppercase tracking-wider font-semibold px-2 py-1 rounded inline-flex items-center gap-1 border border-white/5">
                            <span className="material-symbols-rounded text-[14px] text-orange-400">push_pin</span>
                            {hl}
                          </span>
                        ))}
                      </div>
              ...
              <button
                onClick={() => setStep('form')}
                className="mt-4 w-full bg-white/5 hover:bg-white/10 text-white py-2.5 rounded-xl text-sm font-semibold transition-colors flex items-center justify-center gap-2 border border-white/10"
              >
