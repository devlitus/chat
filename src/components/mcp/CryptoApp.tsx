...
function formatPrice(price: number): string {
  return new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', minimumFractionDigits: 2, maximumFractionDigits: price < 10 ? 4 : 2 }).format(price);
}
      ...
      <div className="relative w-full max-w-sm bg-[#0b1221] rounded-3xl p-[3px] shadow-xl border border-white/5 overflow-hidden">
        <div className="bg-[#0f1523] rounded-[1.35rem] p-5 relative overflow-hidden">
          <div className="flex items-center gap-4 mb-5">
            ...
            <div>
              <h2 className="text-xl font-bold leading-tight" style={{ backgroundClip: 'text', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundImage: 'linear-gradient(to right, #60a5fa, #22d3ee)' }}>Crypto Prices</h2>
              <p className="text-xs font-semibold text-gray-500 tracking-wide uppercase mt-0.5">Coingecko API</p>
            </div>
          </div>
          ...
          {status === 'error' && (() => {
            ...
            return (
              <div className="flex flex-col items-center gap-3 py-6 text-center mb-2">
                ...
                <div><p className="text-gray-300 text-sm font-semibold">{info.title}</p><p className="text-gray-600 text-xs mt-1">{info.detail}</p></div>
                {errorCode === 'rate-limited' && retryIn !== null && (
                  <div className="flex items-center gap-2 bg-[#080c14] border border-white/5 rounded-xl px-4 py-2 mt-1">
                    ...
                    <span className="text-xs text-gray-400">Reintentando en <span className="text-[#22d3ee] font-bold tabular-nums">{retryIn}s</span></span>
                  </div>
                )}
              </div>
            );
          ...
          {status === 'success' && coins.length > 0 && (
            <div className="space-y-2.5 mb-5">
              {coins.map((coin) => {
                ...
                return (
                  <div key={coin.id} className="group bg-[#080c14] hover:bg-[#0d121c] border border-white/5 rounded-2xl p-4 flex items-center justify-between transition-all duration-200 cursor-pointer">
                    <div className="flex items-center gap-3">
                      <div className="text-[#22d3ee]"><span className="material-symbols-rounded text-[28px]">{meta.icon}</span></div>
                      <div><h4 className="text-white font-bold text-sm leading-none">{meta.label}</h4><span className="text-xs text-gray-500 font-medium mt-0.5 block">{meta.symbol}</span></div>
                    </div>
                    <div className="text-right">
                      <div className="text-white font-bold text-lg tracking-tight leading-none">{formatPrice(coin.price)}</div>
                      <div className={`flex items-center justify-end gap-0.5 text-xs font-bold mt-0.5 ${isPos ? 'text-emerald-400' : 'text-red-400'}`}>
                        <span className="material-symbols-rounded text-[16px] leading-none">{isPos ? 'arrow_drop_up' : 'arrow_drop_down'}</span>
          ...
          <div className="relative group mt-5">
            ...
            <button onClick={fetchPrices} disabled={status === 'loading' || retryIn !== null} className="relative w-full bg-[#1e293b]/50 hover:bg-[#1e293b] text-white py-3.5 px-4 rounded-xl flex items-center justify-center gap-2 transition-all duration-200 border border-white/10 shadow-lg disabled:opacity-60 disabled:cursor-not-allowed font-sans">
              <span className={`material-symbols-rounded text-[20px] ${status === 'loading' ? 'animate-spin' : ''}`}>refresh</span>
              <span className="font-bold text-sm tracking-wide uppercase">{status === 'loading' ? 'Actualizando...' : retryIn !== null ? `Reintentando en ${retryIn}s` : 'Actualizar Precios'}</span>
            </button>
          </div>
