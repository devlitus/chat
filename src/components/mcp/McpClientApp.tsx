      ...
      <div className="relative w-full max-w-sm bg-[#0b1221] rounded-3xl p-[3px] shadow-xl border border-white/5 overflow-hidden">
        <div className="bg-[#0f1523] rounded-[1.35rem] p-5 relative overflow-hidden">
          <div className="flex items-center gap-4 mb-5">
            ...
            <div>
              <h2 className="text-xl font-bold leading-tight" style={{ backgroundClip: 'text', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundImage: 'linear-gradient(to right, #60a5fa, #22d3ee)' }}>Live Sync</h2>
              <p className="text-xs font-semibold text-gray-500 tracking-wide uppercase mt-0.5">MCP Tool Demo</p>
            </div>
          </div>

          <div className="mb-5">
            <p className="text-xs font-semibold text-gray-600 tracking-widest uppercase mb-3 pl-1">Tiempo del Servidor</p>
            <div className="bg-[#080c14] border border-white/5 rounded-2xl px-6 py-6 flex items-center justify-center min-h-[88px]">
              <code className={`font-mono text-3xl tracking-widest font-bold transition-all duration-300 ${isFetching ? 'text-[#22d3ee] animate-pulse' : 'text-white'}`}>{serverTime}</code>
            </div>
          </div>
          ...
          <div className="relative group mt-5">
            ...
            <button onClick={handleSync} disabled={isFetching} className="relative w-full bg-[#1e293b]/50 hover:bg-[#1e293b] text-white py-3.5 px-4 rounded-xl flex items-center justify-center gap-2 transition-all duration-200 border border-white/10 shadow-lg disabled:opacity-60 disabled:cursor-not-allowed font-sans">
              <span className={`material-symbols-rounded text-[20px] ${isFetching ? 'animate-spin' : ''}`}>sync</span>
              <span className="font-bold text-sm tracking-wide uppercase">{isFetching ? 'Actualizando...' : 'Sincronizar Ahora'}</span>
            </button>
          </div>
