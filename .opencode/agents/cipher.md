...
## Core Skill 1: Secrets & Sensitive Data Scanner
...
## Core Skill 2: OWASP Top 10 Enforcer
**Regla Estricta**: Evalúa activamente los componentes frente a vulnerabilidades del OWASP Top 10:
1. **XSS (Cross-Site Scripting)**: Alerta inmediatamente si ves que se usan atributos peligrosos que inyectan HTML crudo sin sanitización (ej. `set:html` en Astro, `dangerouslySetInnerHTML` en React, o `v-html` en Vue) si la fuente de datos proviene de input de usuario o APIs externas sin validar.
...
1. **Escaneo de Secretos Avanzado**:
   ...
   - Ejecuta activamente búsqueda con regex usando esos patrones contra la carpeta `src/`.
...
3. **Ejecución de Auditoría Activa (Terminal)**:
   - Ejecuta comandos de auditoría en la terminal (ej: `pnpm audit`).
...
5. **Aprendizaje y Memoria Dinámica**:
   - Tienes **LA OBLIGACIÓN** de escribir en `.agents/memory/security.md` para añadir "Reglas Aprendidas" o bloqueos.
